...
% on on a uniform mesh with N intervals in each coordinate direction.
% Authors: Niall Madden and Stephen Russell, NUI Galway. Sep 2015.
% This code is used to generate results in 
%    Russell, S., and  Madden, N. An analysis and implementation of 
%       sparse grid finite element methods. 
...
% Pick one of 'classical', 'two-scale', 'multiscale'
Method = 'two-scale';
...
u = chebfun2(@(x,y)4*sin(pi*x).*(y-y.^2), [0 1 0 1]);
...
for N = Ns
   ...
   switch Method
      case 'classical' % Classical Galerkin methMultiScale_Projectorod
      ...
      case 'two-scale'    % the two-scale sparse grid FEM
         ...
         A = P'*A*P;
         A = (A+A')/2;   % Sometimes, due to round-off errors, backslash
         % does not notice that the matrix is symmetric.
      case 'multiscale' % the multiscale FEM
