...
void GaussSeidel(double **A, double *b, double *xk, unsigned N,
            unsigned &count, unsigned max_its, double TOL);
...
int main(void )
{
  ...
  GaussSeidel(A,b,xk,N, it_count, max_its, TOL);
  std::cout << "GS Estimate is : " << std::endl;
  print_vec(xk, N);
  std::cout << std::endl;
  std::cout << "GS " << it_count << " iterations" << std::endl;
  std::cout << "GS Error is " << diff(xk,x,N) << std::endl;
  return (0);
}
