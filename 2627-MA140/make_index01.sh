#!/bin/bash
# Generate thumbnails + index.html using PDF metadata titles
# - Normal PDFs (*.pdf but not *.pdf.pdf) => "Slides"
# - Annotated slides (*.pdf.pdf) => "Annotations"
# Uses index_head.html for the HTML <head> and beginning of <body>

command -v pdftoppm >/dev/null 2>&1 || { 
  echo "Error: pdftoppm (poppler-utils) not installed. Run: sudo apt-get install poppler-utils"
  exit 1
}
command -v pdfinfo >/dev/null 2>&1 || { 
  echo "Error: pdfinfo (poppler-utils) not installed. Run: sudo apt-get install poppler-utils"
  exit 1
}

mkdir -p thumbs

# === Generate thumbnails ===
for f in *.pdf; do
    base="${f%.pdf}"
    if [ ! -f "thumbs/$base.png" ]; then
        echo "Creating thumbnail for $f..."
        pdftoppm -png -scale-to 200 -f 1 -singlefile "$f" "thumbs/$base"
    fi
done

# === Start HTML with custom head ===
if [ ! -f index_head.html ]; then
  echo "Error: index_head.html not found!"
  exit 1
fi
cat index_head.html > index.html

# === Slides section ===
echo "  <h1 id=\"slides\">Slides</h1>" >> index.html
echo "  <div class=\"grid\">" >> index.html

for f in *.pdf; do
    # Skip annotated slides (*.pdf.pdf)
    if [[ "$f" == *.pdf.pdf || "$f" == *_annotated.pdf ]]; then continue; fi
    base="${f%.pdf}"
    # Extract Title metadata, split at " - " if present
    title=$(pdfinfo "$f" | grep '^Title:' | sed 's/^Title:[[:space:]]*//' | sed 's/ - .*//')
    if [ -z "$title" ]; then
        title="$f"
    fi
    echo "    <div class=\"item\"><a href=\"$f\"><img src=\"thumbs/$base.png\" alt=\"$f\"><div class=\"label\">$title</div></a></div>" >> index.html
done

echo "  </div>" >> index.html

# === Annotations section ===
echo "  <h1 id=\"annotations\">Annotations</h1>" >> index.html
echo "  <div class=\"grid\">" >> index.html

for f in *.pdf.pdf *_annotated.pdf; do  [ -e "$f" ] || continue
    base="${f%.pdf}"
    #title=$(pdfinfo "$f" | grep '^Title:' | sed 's/^Title:[[:space:]]*//' | sed 's/ - .*//')
    #if [ -z "$title" ]; then
    #    title="$f"
    #fi
    title="$f"
    echo "    <div class=\"item\"><a href=\"$f\"><img src=\"thumbs/$base.png\" alt=\"$f\"><div class=\"label\">$title</div></a></div>" >> index.html
done

echo "  </div>" >> index.html

# === Footer ===
cat >> index.html <<'EOF'
</body>
</html>
EOF

echo "✅ index.html generated. Open it in your browser!"
