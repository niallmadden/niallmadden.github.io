...
INFILE = 'BAIL-locations.xlsx'
OUTFILE = 'BAILMap.html'
...
folium.CircleMarker(location=[Home.latitude, Home.longitude],
                    ...
                    color="black",
                    fill_opacity=0.6,fill_color="green",
                    fill = False, # overridden by fill_color
                    radius=15).add_to(marker_cluster)
...
for i in range(len(Institutes)):
    ...
    if (inst in Done):
        ...
        popup_text = inst
        folium.CircleMarker(location = [lat,lon],  tooltip=popup_text,popup= popup_text, fill =True, radius=20).add_to(marker_cluster)
